...
def extract(timeseries: pd.DataFrame, max_timeshift: int, min_timeshift: int) -> (pd.DataFrame, pd.Series):
    ...
    holidays = pd.Series(np.zeros(timeseries.shape[0]),index = timeseries.index)
    holidays[timeseries.Balance == 0] = 1
    ...
    X = add_taxes(X,holidays)
    X['is_holiday'] = holidays
    X = add_macro(X)
    return X, y
...
def add_taxes(timeseries: pd.DataFrame,holidays : pd.Series):
    print('Adding taxes')
    ...
    for col,df in cols.items():
        timeseries[col] = 0
        timeseries.loc[np.isin(timeseries.index,df),col] = 1
        timeseries[col] = smoothing(timeseries[col], 4) 
        timeseries.loc[holidays==1,col] = 0
    return timeseries

def add_macro(X : pd.DataFrame):
    print('adding macro')
    brent = yf.download("BZ=F", start="2017-01-09", end="2021-03-31")
    moex = yf.download("IMOEX.ME", start="2017-01-09", end="2021-03-31")
    usd_rub = yf.download("RUB=X", start="2017-01-09", end="2021-03-31")
